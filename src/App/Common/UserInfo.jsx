import {Link} from "react-router-dom";
import logo from "../../img/header/imageUserUndefined.jpg";
import {useDispatch, useSelector} from "react-redux";
import {follow, getCurrentUserData, unfollow} from "../Store/users";
import {Button} from "react-bootstrap";

export const UserInfo = ({p}) => {
    const dispatch = useDispatch()
    const currentUserData = useSelector(getCurrentUserData())
    const followers = Object.values(currentUserData.followers)
    const followUser = (id) => {
        dispatch(follow(id))
    }
    const unfollowUser = (id) => {
        dispatch(unfollow(id))
    }
    return (
        <tbody key={p._id}>
        <tr>
            <td scope="row" className={"text-center"}>
                <Link to={'/chat'}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="23" height="23" fill="currentColor"
                         className="bi bi-chat" viewBox="0 0 16 16">
                        <path
                            d="M2.678 11.894a1 1 0 0 1 .287.801 10.97 10.97 0 0 1-.398 2c1.395-.323 2.247-.697 2.634-.893a1 1 0 0 1 .71-.074A8.06 8.06 0 0 0 8 14c3.996 0 7-2.807 7-6 0-3.192-3.004-6-7-6S1 4.808 1 8c0 1.468.617 2.83 1.678 3.894zm-.493 3.905a21.682 21.682 0 0 1-.713.129c-.2.032-.352-.176-.273-.362a9.68 9.68 0 0 0 .244-.637l.003-.01c.248-.72.45-1.548.524-2.319C.743 11.37 0 9.76 0 8c0-3.866 3.582-7 8-7s8 3.134 8 7-3.582 7-8 7a9.06 9.06 0 0 1-2.347-.306c-.52.263-1.639.742-3.468 1.105z"/>
                    </svg>
                </Link>
            </td>
            <td className={"d-flex text-center"}>
                <img className={"m-2"} style={{
                    width: '40px', height: '40px', borderRadius: '50%', lineHeight: '0', margin: '2px'
                }} src={p.image ? p.image : logo}/>
                <div style={{marginTop: '10px'}}>
                    <p style={{fontSize: '12px', fontWeight: '500', lineHeight: '0'}}>{p.name}</p>
                    <p style={{fontSize: '12px'}}>{p.telephone}</p>
                </div>
            </td>
            <td className={"text-center"}>{p.transfer ? p.transfer : "Нет переводов"}</td>
            <td className={"text-center"}>{p.transferSuccess ? p.transferSuccess : "Нет переводов"}</td>
            <td className={"text-center"} style={{fontSize: '12px', width: '140px'}}>
                {p.location}
            </td>
            <td className={"text-center"}>
                {followers.find((f) => f === p._id)
                    ? <Button onClick={() => unfollowUser(p._id)}>Удалить из друзей</Button>
                    : <Button onClick={() => followUser(p._id)}>Добавить в друзья</Button>
                }
            </td>
        </tr>
        </tbody>
    )
}